...
export class InputMask {
  ...
  constructor(elements, options) {
    this.formatCharacters = ["-", "_", "(", ")", "[", "]", ":", ".", ",", "$", "%", "@", " ", "/"];
    this.maskCharacters = ["A", "9", "*"];
    this.originalValue = "";
    this.mask = null;
    this.hasMask = false;
    ...
    if (options.mask && options.mask.mask && options.mask.mask.length > 0) {
      this.mask = options.mask.mask.split("");
      this.hasMask = true;
    }
    if (options.mask && options.mask.patterns && options.mask.patterns.length > 0) {
      this.maskPatterns = options.mask.patterns;
    }
